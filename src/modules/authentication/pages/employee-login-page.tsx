...
export function EmployeeLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const loggedUser = await FirebaseAuthService.login(email, password, 'employee');
      setUser(loggedUser);
    ...
    <div className="flex flex-col gap-6 text-right w-full">
      ...
      <div className="flex flex-col gap-1 text-right">
        ...
        <p className="text-[11px] text-slate-400 mt-0.5">
          Enter your email and password (بوابة الموظفين وفريق العمل)
        </p>
      </div>
      ...
      <div className="flex items-center justify-center gap-6 text-[11px] text-slate-500 pt-5 border-t border-slate-900">
        <Link to="/auth/admin-login" className="hover:text-blue-500 transition-colors duration-300 relative group py-1 font-bold">
